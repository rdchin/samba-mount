...
##             "--about", and "--history".
##            *f_help_message added to allow the argument "--help"
##             to invoke a help message.
##            *f_main_menu_txt fixed bug in case statement.
##
...
##
## 2016-12-11 *f_mount_txt fixed faulty code by total rewrite by pseudo-code.
...
f_help_message () {
      echo
      echo "Usage: bash mountup.sh"
      echo "       bash mountup.sh [SAMBA_FILE_SERVER_NAME]"
      echo "       bash mountup.sh --help     # Displays this help message."
      echo "       bash mountup.sh --version  # Displays script version."
      echo "       bash mountup.sh --about    # Displays script version."
      echo "       bash mountup.sh --history  # Displays script code history."
      echo
      echo "Mount shared folders on a Samba file server."
      echo
      echo "The optional argument [SAMBA_FILE_SERVER_NAME] will attempt to mount"
      echo "all share-points (shared folders) for that Samba file server."
      echo
