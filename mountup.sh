...
if [ -r mountup.lib ] ; then  # Does library file exist and is readable in the same directory as this script.
   ...
   f_message $GUI "OK" "Missing a required file" "Missing a required file: \"mountup.lib\" from this directory.\n\n                    *** Aborting program ***"
   #$GUI --infobox "Missing a required file: \"mountup.lib\" from this directory.\n\n                    *** Aborting program ***" 5 71
   exit 0  # This cleanly closes the process generated by #!bin/bash. 
           # Otherwise every time this script is run, another instance of
           # process /bin/bash is created using up resources.
fi
